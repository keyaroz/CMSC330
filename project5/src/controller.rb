...
module Tokens
	...
	def issue_token
		n = 5
		SecureRandom.random_number(n ** SESSION_BITS)
	end
...
module Access
	...
	def authenticate(user, passwd)
		...
		for i in pw_arr
			...
			assign_session user if get_user(user) && x[:password] == passwd
	...
	def revoke(user, session)
		if (session.is_a?(Integer) == false) then 
			false 
		end
...
module User
		...
		def search(user_query)
			...
			q_arr = user_query.chars
			user_query = ""
			for i in q_arr
				if !(i == "\;" || i == "." || i == "\/" || i == ">" || i == "<" || i == "\'") then 
					user_query = user_query + i
				end
			end		# end for	
	...
	def update_prefs(user, session, description, token)
		return false if (session.is_a?(Integer) == false)

      	return false if (token.is_a?(Integer) == false)
		...
		return false if   @hashy[user] != token
		return false if  @sessions[user] != session
