...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  | Char(a) -> let st = fresh() in let f_st = fresh() in 
  {
    q0 = st;
    qs = [st ; f_st];
    sigma = [a];
    fs = [f_st];
    delta = [(st, Some a, f_st)];
  }

  | Union (exp1, exp2) ->
    let left = regexp_to_nfa exp1 in let right = regexp_to_nfa exp2 in 
    let st = fresh() in let f_st = fresh() in
  
    {
      q0 = st;
      qs = Sets.union left.qs (Sets.union right.qs [st;f_st]);
      sigma = Sets.union (left.sigma) (right.sigma);
      fs = [f_st];
      delta = Sets.union left.delta ( Sets.union right.delta [((hd right.fs), None, f_st) ; (st, None, right.q0) ; ((hd left.fs), None, f_st) ; (st, None, left.q0)]);
    }
  ...
  | Concat (exp1,exp2) -> 
    ...
    {
      q0 = left.q0;  
      qs = Sets.union left.qs right.qs;
      sigma = Sets.union left.sigma right.sigma;
      fs = right.fs;
      delta =  Sets.union (Sets.union left.delta right.delta)  [((hd left.fs), None, right.q0)];

    }


  | Star (exp) ->
    let rec1 = regexp_to_nfa exp in
    let st = fresh() in
    let f_st = fresh() in
     { q0 = st;  
      qs = Sets.union rec1.qs [st;f_st];
      sigma = rec1.sigma;
      fs = [f_st];
      delta =  Sets.union rec1.delta [((hd rec1.fs), None, f_st) ; (st, None, rec1.q0) ; (st, None, f_st) ; (f_st, None, st)];
     }
